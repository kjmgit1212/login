...
<body>
	...
	<form action="${contextPath}/freeBoard/saveFreeBoard" method="post">
		<input type="text" name="writer" placeholder="작성자" value="${user.id}" readonly>
	...
	<table>
		<caption>${totalRecord - fb.rowNum + 1}개 게시글</caption>
		<thead>
			<tr>
				<td>번호</td>
				<td>작성자</td>
				...
				<c:forEach var="fb" items="${freeBoards}">
					...
					<c:if test="${fb.state == 1}">
						<tr>
							...
							<td>${fb.created}</td>
							<td>
								<c:if test="${user.id eq fb.writer}">
						...
						<tr class="reply_form blind">
							<td colspan="5">
								<form action="${contextPath}/freeBoard/saveReply" method="post">
									<input type="text" name="writer" placeholder="작성자" size="4" value="${user.id}" readonly>
